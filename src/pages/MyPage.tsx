...
export function MyPage() {
  ...
  return <div className="my-page"><Header white /><main className="page-shell my-shell"><BackLink to="/" />
    ...
    <DashboardSection title={locale === "ko" ? "최근 본 항목" : "Recently viewed"} description={locale === "ko" ? "최근 열어 본 종목, 뉴스와 공시를 순서대로 표시합니다." : "Your recent activity, automatically organized for quick access."}><RemoteState {...recent} empty={(value) => !value.length} emptyMessage={locale === "ko" ? "아직 열어 본 항목이 없습니다." : "No recently viewed items yet."}>{(items) => <ActivityList items={items} />}</RemoteState></DashboardSection>
    ...
    <DashboardSection title={locale === "ko" ? "통합 인텔리전스 피드" : "Integrated intelligence feed"} description={locale === "ko" ? "관심종목과 관련된 뉴스와 공시만 표시합니다." : "News and filings filtered to your watchlist only."}><RemoteState {...feed} empty={(value) => !value.length} emptyMessage={locale === "ko" ? "관심종목을 추가하면 관련 인텔리전스를 보여드립니다." : "Add a watchlist company to see its intelligence feed."}>{(items) => <ActivityList items={items} />}</RemoteState></DashboardSection>
