...
export function MyPage() {
  ...
  return <div className="my-page"><Header white /><main className="page-shell my-shell"><BackLink to="/" />
    ...
    <DashboardSection title="Integrated Intelligence Feed" description="News and filings filtered to your watchlist only."><RemoteState {...feed} empty={(value) => !value.length}>{(items) => <ActivityList items={items.map((item): Activity => item.type === "NEWS" ? { type: "NEWS", id: item.news.id, title: item.news.englishTitle || "Translation unavailable", date: item.date } : { type: "FILING", id: item.filing.receiptNumber, title: item.filing.titleEn || "Translation unavailable", date: item.date })} />}</RemoteState></DashboardSection>
