...
- 모든 세율 확인 진입점은 같은 우측 Tax eligibility K-Agent를 사용한다. 이전 `/tax` 주소도 홈과 공통 드로어를 표시하며 세율·가이드 단계·문서 상태·비교 결과는 Backend API에서 가져온다. 드로어 폭은 Figma 비율로 반응하며 상세 가이드에서 최신 제한세율 적용신청서 원본을 내려받을 수 있다.
