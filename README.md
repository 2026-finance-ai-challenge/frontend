...
- 홈과 뉴스 목록은 같은 중요도 정렬 API를 사용한다. 뉴스 목록은 하단 접근 시 다음 커서를 조회하며 로딩 중 스켈레톤을 표시한다.
...
- 브라우저 스크롤 복원을 초기 실행부터 끄고 화면 전환의 첫 그리기 전에 즉시 상단을 설정한다. 페이지 이동에 스크롤 애니메이션을 적용하지 않는다.
