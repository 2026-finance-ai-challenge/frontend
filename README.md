...
- What/Why/Impact는 하나의 공유 생성 작업에서 EN/KR 필드를 각각 저장한다. 캐시가 없을 때만 호버 후 생성을 요청하고 완료 시 새로고침 없이 갱신하며 언어 전환으로 재생성하지 않는다.
...
- 모든 세율 확인 진입점은 같은 우측 Tax eligibility K-Agent를 사용한다. 이전 `/tax` 주소도 홈과 공통 드로어를 표시하며 세율·가이드 단계·문서 상태·비교 결과는 Backend API에서 가져온다. 일반·뉴스·공시·세무·대화 기록 드로어는 Figma 비율의 공통 폭과 반응형 글자·행간·말풍선 간격을 사용한다. 상세 가이드에서는 최신 제한세율 적용신청서 원본을 내려받을 수 있다.
- 홈·전체 외국인 한도 목록·종목 상세 게이지는 법정 제한 4종목의 예측 범위와 기준점을 직전 보유율에 함께 표시한다. 정규장에는 장중 예측, 장외에는 Backend가 검증한 다음 개장 세션 예측과 대상일을 구분해 표시한다. 대상 세션 예측이 없으면 직전 보유율만 유지하며 전일 수치를 다음 거래일 예측으로 바꾸거나 값을 생성하지 않는다.
- 예측은 화면이 보일 때 장중 1분·장외 5분 간격으로 다시 조회하며, 백그라운드 탭에서는 중단하고 복귀·개장·마감 전환 시 갱신한다.
