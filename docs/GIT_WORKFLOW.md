...
3. 배포 시 `develop`에서 `main` 대상 PR을 생성하고 검사를 통과한 뒤 스쿼시 병합한다.
4. `develop -> main` 배포 PR에서는 `--delete-branch`를 절대 사용하지 않는다.
5. 병합 직후 원격 `develop` 존재 여부를 확인한다.

영구 브랜치 이력이 갈라져 동기화가 필요하면 별도 동기화 PR을 만들고, 사용자의 명시적 승인을 받은 그 PR에만 merge commit을 사용한다. 일반 작업·배포 PR에 이 예외를 적용하지 않는다.
